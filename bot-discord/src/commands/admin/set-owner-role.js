...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Define o cargo owner usado para configuracoes criticas.')
    .setDMPermission(false)
    .addRoleOption((option) =>
      option.setName('role').setDescription('Cargo do nivel owner').setRequired(true)
    ),
  async execute(interaction) {
    ...
    await interaction.reply({
      embeds: [
        successEmbed(
          interaction.guild,
          'Cargo owner atualizado',
          `${role} agora sera usado para configuracoes criticas, pagamentos e mapeamento de cargos.`
        )
      ],
      ephemeral: true
    });
